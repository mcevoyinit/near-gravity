/*
 * NEAR Semantic Guard Smart Contract - Complete Implementation
 * 
 * This contract demonstrates the full integration between NearGravity's RAG system
 * and NEAR blockchain for semantic analysis verification and storage.
 * 
 * Integrates with:
 * - src/models/dto/rag_models.rs (RAG data transfer objects)
 * - src/services/ai/semantic_service.rs (semantic analysis)
 * - src/matching_engine/ (similarity algorithms)
 */

use near_sdk::borsh::{self, BorshDeserialize, BorshSerialize};
use near_sdk::collections::{LookupMap, UnorderedMap};
use near_sdk::serde::{Deserialize, Serialize};
use near_sdk::{env, near_bindgen, AccountId, PanicOnDefault, Promise, CryptoHash};
use std::collections::HashMap;

// ============================================================================
// NEARGRAVITY DTO INTEGRATION
// ============================================================================
// These types mirror the data structures from NearGravity's core system

/// Mirrors src/models/dto/rag_models.rs::SemanticDelta
#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct SemanticDelta {
    pub similarity_score: f64,
    pub confidence_level: f64,
    pub transformation_type: String,
    pub semantic_distance: f64,
    pub integrity_verified: bool,
}

/// Mirrors src/models/dto/rag_models.rs::MessageEmbedding
#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct MessageEmbedding {
    pub vector: Vec<f64>,
    pub model_name: String,
    pub embedding_hash: String,
    pub semantic_hash: String,
    pub timestamp: u64,
}

/// Mirrors src/services/ai/semantic_service.rs patterns
#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct SemanticAnalysisResult {
    pub center_of_gravity: String,
    pub outliers: Vec<SemanticOutlier>,
    pub distance_matrix: HashMap<String, f64>,
    pub embeddings: Vec<MessageEmbedding>,
    pub threshold_used: f64,
    pub processing_time_ms: u64,
    pub consensus_score: f64,
}

/// Extends NearGravity's outlier detection with blockchain verification
#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct SemanticOutlier {
    pub result_id: String,
    pub reason: String,
    pub severity: OutlierSeverity,
    pub max_distance: f64,
    pub source_type: String,
    pub verification_status: VerificationStatus,
    pub outlier_distances: Vec<OutlierDistance>,
}

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct OutlierDistance {
    pub to_result: String,
    pub distance: f64,
    pub threshold_exceeded_by: f64,
}

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub enum OutlierSeverity {
    Low,
    Medium,
    High,
    Critical,
}

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub enum VerificationStatus {
    Pending,
    Verified,
    Disputed,
    Flagged,
}

/// Search result with semantic metadata
#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct SearchResult {
    pub id: String,
    pub title: String,
    pub snippet: String,
    pub url: String,
    pub rank: u32,
    pub source_type: String,
    pub semantic_hash: String,
    pub trustworthiness_score: f64,
}

/// Complete semantic guard analysis record
#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct SemanticGuardRecord {
    pub id: String,
    pub query: String,
    pub results: Vec<SearchResult>,
    pub semantic_analysis: SemanticAnalysisResult,
    pub submitter: AccountId,
    pub timestamp: u64,
    pub block_height: u64,
    pub metadata: AnalysisMetadata,
}

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct AnalysisMetadata {
    pub model_version: String,
    pub algorithm_version: String,
    pub processing_node: String,
    pub verification_count: u32,
    pub dispute_count: u32,
    pub consensus_reached: bool,
}

// ============================================================================
// PREDICTION MARKET INTEGRATION
// ============================================================================
// Extends semantic analysis with prediction market capabilities

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct SemanticPredictionMarket {
    pub market_id: String,
    pub question: String,
    pub resolution_criteria: String,
    pub semantic_threshold: f64,
    pub total_stake: u128,
    pub positions: HashMap<AccountId, MarketPosition>,
    pub status: MarketStatus,
    pub created_at: u64,
    pub resolution_time: Option<u64>,
}

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct MarketPosition {
    pub prediction: SemanticPrediction,
    pub stake_amount: u128,
    pub confidence: f64,
    pub timestamp: u64,
}

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub enum SemanticPrediction {
    HighConsensus,    // All sources will align
    Misinformation,   // Outliers contain false information
    MixedSignals,     // Legitimate disagreement
    Insufficient,     // Not enough data
}

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub enum MarketStatus {
    Active,
    Resolved,
    Disputed,
    Cancelled,
}

// ============================================================================
// ORACLE SYSTEM INTEGRATION
// ============================================================================
// Multi-agent oracle system for consensus building

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct OracleSubmission {
    pub oracle_id: AccountId,
    pub semantic_analysis: SemanticAnalysisResult,
    pub confidence_score: f64,
    pub stake_amount: u128,
    pub timestamp: u64,
    pub verification_proof: String,
}

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct ConsensusState {
    pub submissions: Vec<OracleSubmission>,
    pub consensus_analysis: Option<SemanticAnalysisResult>,
    pub agreement_score: f64,
    pub dispute_period_end: u64,
    pub finalized: bool,
}

// ============================================================================
// MAIN CONTRACT
// ============================================================================

#[near_bindgen]
#[derive(BorshDeserialize, BorshSerialize, PanicOnDefault)]
pub struct SemanticGuardContract {
    /// Stored semantic analysis records
    pub analyses: UnorderedMap<String, SemanticGuardRecord>,
    
    /// Active prediction markets
    pub prediction_markets: UnorderedMap<String, SemanticPredictionMarket>,
    
    /// Oracle consensus tracking
    pub consensus_states: UnorderedMap<String, ConsensusState>,
    
    /// Registered oracle nodes
    pub oracles: LookupMap<AccountId, OracleInfo>,
    
    /// Global contract metadata
    pub contract_metadata: ContractMetadata,
}

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct OracleInfo {
    pub stake_amount: u128,
    pub reputation_score: f64,
    pub submissions_count: u64,
    pub accuracy_rate: f64,
    pub active: bool,
}

#[derive(BorshDeserialize, BorshSerialize, Serialize, Deserialize, Clone)]
#[serde(crate = "near_sdk::serde")]
pub struct ContractMetadata {
    pub version: String,
    pub total_analyses: u64,
    pub total_markets: u64,
    pub total_staked: u128,
    pub consensus_threshold: f64,
}

#[near_bindgen]
impl SemanticGuardContract {
    #[init]
    pub fn new() -> Self {
        Self {
            analyses: UnorderedMap::new(b"a"),
            prediction_markets: UnorderedMap::new(b"m"),
            consensus_states: UnorderedMap::new(b"c"),
            oracles: LookupMap::new(b"o"),
            contract_metadata: ContractMetadata {
                version: "1.0.0".to_string(),
                total_analyses: 0,
                total_markets: 0,
                total_staked: 0,
                consensus_threshold: 0.75,
            },
        }
    }

    // ========================================================================
    // SEMANTIC ANALYSIS STORAGE
    // ========================================================================

    /// Submit semantic analysis results to blockchain for verification
    #[payable]
    pub fn submit_semantic_analysis(
        &mut self,
        query: String,
        results: Vec<SearchResult>,
        semantic_analysis: SemanticAnalysisResult,
        metadata: AnalysisMetadata,
    ) -> String {
        let analysis_id = self.generate_analysis_id(&query);
        
        let record = SemanticGuardRecord {
            id: analysis_id.clone(),
            query,
            results,
            semantic_analysis,
            submitter: env::predecessor_account_id(),
            timestamp: env::block_timestamp(),
            block_height: env::block_index(),
            metadata,
        };

        self.analyses.insert(&analysis_id, &record);
        self.contract_metadata.total_analyses += 1;

        env::log_str(&format!("Semantic analysis stored: {}", analysis_id));
        analysis_id
    }

    /// Retrieve semantic analysis by ID
    pub fn get_semantic_analysis(&self, analysis_id: String) -> Option<SemanticGuardRecord> {
        self.analyses.get(&analysis_id)
    }

    /// Get analyses with outliers above threshold
    pub fn get_high_risk_analyses(&self, severity_threshold: OutlierSeverity) -> Vec<SemanticGuardRecord> {
        self.analyses
            .values()
            .filter(|record| {
                record.semantic_analysis.outliers.iter().any(|outlier| {
                    matches!(
                        (&outlier.severity, &severity_threshold),
                        (OutlierSeverity::Critical, _) |
                        (OutlierSeverity::High, OutlierSeverity::High | OutlierSeverity::Medium | OutlierSeverity::Low) |
                        (OutlierSeverity::Medium, OutlierSeverity::Medium | OutlierSeverity::Low) |
                        (OutlierSeverity::Low, OutlierSeverity::Low)
                    )
                })
            })
            .collect()
    }

    // ========================================================================
    // PREDICTION MARKET OPERATIONS
    // ========================================================================

    /// Create prediction market for semantic consensus
    #[payable]
    pub fn create_prediction_market(
        &mut self,
        question: String,
        resolution_criteria: String,
        semantic_threshold: f64,
    ) -> String {
        let market_id = self.generate_market_id(&question);
        
        let market = SemanticPredictionMarket {
            market_id: market_id.clone(),
            question,
            resolution_criteria,
            semantic_threshold,
            total_stake: 0,
            positions: HashMap::new(),
            status: MarketStatus::Active,
            created_at: env::block_timestamp(),
            resolution_time: None,
        };

        self.prediction_markets.insert(&market_id, &market);
        self.contract_metadata.total_markets += 1;

        market_id
    }

    /// Place prediction on semantic outcome
    #[payable]
    pub fn place_prediction(
        &mut self,
        market_id: String,
        prediction: SemanticPrediction,
        confidence: f64,
    ) {
        let stake_amount = env::attached_deposit();
        assert!(stake_amount > 0, "Must attach stake");

        let mut market = self.prediction_markets.get(&market_id)
            .expect("Market not found");
        
        assert!(matches!(market.status, MarketStatus::Active), "Market not active");

        let position = MarketPosition {
            prediction,
            stake_amount,
            confidence,
            timestamp: env::block_timestamp(),
        };

        market.positions.insert(env::predecessor_account_id(), position);
        market.total_stake += stake_amount;
        
        self.prediction_markets.insert(&market_id, &market);
        self.contract_metadata.total_staked += stake_amount;
    }

    /// Resolve prediction market based on semantic analysis
    pub fn resolve_market(&mut self, market_id: String, analysis_id: String) {
        let mut market = self.prediction_markets.get(&market_id)
            .expect("Market not found");
        
        let analysis = self.analyses.get(&analysis_id)
            .expect("Analysis not found");

        // Determine actual outcome based on semantic analysis
        let actual_outcome = self.determine_semantic_outcome(&analysis.semantic_analysis);
        
        // Distribute rewards to correct predictors
        self.distribute_market_rewards(&mut market, actual_outcome);
        
        market.status = MarketStatus::Resolved;
        market.resolution_time = Some(env::block_timestamp());
        
        self.prediction_markets.insert(&market_id, &market);
    }

    // ========================================================================
    // ORACLE CONSENSUS SYSTEM
    // ========================================================================

    /// Register as oracle node
    #[payable]
    pub fn register_oracle(&mut self) {
        let stake_amount = env::attached_deposit();
        assert!(stake_amount >= 1_000_000_000_000_000_000_000_000, "Minimum 1 NEAR stake required");

        let oracle_info = OracleInfo {
            stake_amount,
            reputation_score: 1.0,
            submissions_count: 0,
            accuracy_rate: 0.0,
            active: true,
        };

        self.oracles.insert(&env::predecessor_account_id(), &oracle_info);
    }

    /// Submit oracle analysis for consensus building
    #[payable]
    pub fn submit_oracle_analysis(
        &mut self,
        analysis_id: String,
        semantic_analysis: SemanticAnalysisResult,
        confidence_score: f64,
        verification_proof: String,
    ) {
        assert!(self.oracles.get(&env::predecessor_account_id()).is_some(), "Not registered oracle");
        
        let submission = OracleSubmission {
            oracle_id: env::predecessor_account_id(),
            semantic_analysis,
            confidence_score,
            stake_amount: env::attached_deposit(),
            timestamp: env::block_timestamp(),
            verification_proof,
        };

        let mut consensus_state = self.consensus_states.get(&analysis_id)
            .unwrap_or_else(|| ConsensusState {
                submissions: Vec::new(),
                consensus_analysis: None,
                agreement_score: 0.0,
                dispute_period_end: env::block_timestamp() + 86400000000000, // 24 hours
                finalized: false,
            });

        consensus_state.submissions.push(submission);
        
        // Check if consensus reached
        if consensus_state.submissions.len() >= 3 {
            self.calculate_consensus(&mut consensus_state);
        }

        self.consensus_states.insert(&analysis_id, &consensus_state);
    }

    // ========================================================================
    // INTERNAL HELPER METHODS
    // ========================================================================

    fn generate_analysis_id(&self, query: &str) -> String {
        let timestamp = env::block_timestamp();
        let hash_input = format!("{}{}{}", query, timestamp, env::predecessor_account_id());
        let hash = env::sha256(hash_input.as_bytes());
        near_sdk::bs58::encode(hash).into_string()
    }

    fn generate_market_id(&self, question: &str) -> String {
        let timestamp = env::block_timestamp();
        let hash_input = format!("{}{}", question, timestamp);
        let hash = env::sha256(hash_input.as_bytes());
        near_sdk::bs58::encode(hash).into_string()
    }

    fn determine_semantic_outcome(&self, analysis: &SemanticAnalysisResult) -> SemanticPrediction {
        if analysis.outliers.is_empty() {
            SemanticPrediction::HighConsensus
        } else if analysis.outliers.iter().any(|o| matches!(o.severity, OutlierSeverity::Critical | OutlierSeverity::High)) {
            SemanticPrediction::Misinformation
        } else if analysis.outliers.len() > analysis.embeddings.len() / 2 {
            SemanticPrediction::MixedSignals
        } else {
            SemanticPrediction::Insufficient
        }
    }

    fn distribute_market_rewards(&self, market: &mut SemanticPredictionMarket, actual_outcome: SemanticPrediction) {
        // Implementation for reward distribution based on correct predictions
        // This would involve calculating payouts and transferring tokens
        env::log_str(&format!("Market {} resolved with outcome: {:?}", market.market_id, actual_outcome));
    }

    fn calculate_consensus(&self, consensus_state: &mut ConsensusState) {
        // Implementation for multi-oracle consensus algorithm
        // Would compare semantic analyses and determine agreement score
        let agreement_threshold = 0.8;
        
        if consensus_state.agreement_score >= agreement_threshold {
            consensus_state.finalized = true;
            env::log_str("Consensus reached");
        }
    }

    // ========================================================================
    // PUBLIC QUERY METHODS
    // ========================================================================

    pub fn get_contract_stats(&self) -> ContractMetadata {
        self.contract_metadata.clone()
    }

    pub fn get_oracle_info(&self, oracle_id: AccountId) -> Option<OracleInfo> {
        self.oracles.get(&oracle_id)
    }

    pub fn get_active_markets(&self) -> Vec<SemanticPredictionMarket> {
        self.prediction_markets
            .values()
            .filter(|market| matches!(market.status, MarketStatus::Active))
            .collect()
    }

    pub fn get_consensus_state(&self, analysis_id: String) -> Option<ConsensusState> {
        self.consensus_states.get(&analysis_id)
    }
}

// ============================================================================
// TESTS
// ============================================================================

#[cfg(test)]
mod tests {
    use super::*;
    use near_sdk::test_utils::{accounts, VMContextBuilder};
    use near_sdk::{testing_env, MockedBlockchain};

    fn get_context(predecessor_account_id: AccountId) -> VMContextBuilder {
        let mut builder = VMContextBuilder::new();
        builder
            .current_account_id(accounts(0))
            .signer_account_id(predecessor_account_id.clone())
            .predecessor_account_id(predecessor_account_id);
        builder
    }

    #[test]
    fn test_semantic_analysis_storage() {
        let context = get_context(accounts(1));
        testing_env!(context.build());

        let mut contract = SemanticGuardContract::new();
        
        let results = vec![SearchResult {
            id: "A".to_string(),
            title: "Test Result".to_string(),
            snippet: "Test snippet".to_string(),
            url: "https://test.com".to_string(),
            rank: 1,
            source_type: "scientific".to_string(),
            semantic_hash: "hash123".to_string(),
            trustworthiness_score: 0.9,
        }];

        let analysis = SemanticAnalysisResult {
            center_of_gravity: "A".to_string(),
            outliers: vec![],
            distance_matrix: HashMap::new(),
            embeddings: vec![],
            threshold_used: 0.75,
            processing_time_ms: 100,
            consensus_score: 0.95,
        };

        let metadata = AnalysisMetadata {
            model_version: "1.0".to_string(),
            algorithm_version: "1.0".to_string(),
            processing_node: "node1".to_string(),
            verification_count: 0,
            dispute_count: 0,
            consensus_reached: false,
        };

        let analysis_id = contract.submit_semantic_analysis(
            "test query".to_string(),
            results,
            analysis,
            metadata,
        );

        assert!(contract.get_semantic_analysis(analysis_id).is_some());
        assert_eq!(contract.contract_metadata.total_analyses, 1);
    }

    #[test]
    fn test_prediction_market_creation() {
        let context = get_context(accounts(1));
        testing_env!(context.build());

        let mut contract = SemanticGuardContract::new();
        
        let market_id = contract.create_prediction_market(
            "Will this query show high semantic consensus?".to_string(),
            "Resolved based on outlier count".to_string(),
            0.75,
        );

        assert!(contract.prediction_markets.get(&market_id).is_some());
        assert_eq!(contract.contract_metadata.total_markets, 1);
    }
}